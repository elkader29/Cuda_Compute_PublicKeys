...
/**************************** VARIABLES *****************************/
__constant__ static const WORD k_sha256[64] = {
...
__device__ inline void SHA256::transform(const unsigned char *message, unsigned int block_nb)
{
	...
	for (i = 0; i < (int) block_nb; i++) {
		...
		for (int j = 0; j < 64; j++) {
			t1 = wv[7] + EP1(wv[4]) + CH(wv[4], wv[5], wv[6]) + k_sha256[j] + w[j];
...
__device__ inline SHA256::SHA256()
{
	m_h[0] = 0x6a09e667;
	m_h[1] = 0xbb67ae85;
	m_h[2] = 0x3c6ef372;
...
__device__ inline void SHA256::update(const unsigned char *message, unsigned int len)
{
...
__device__ inline void SHA256::final(unsigned char *digest)
{
	unsigned int block_nb;
	unsigned int pm_len;
	unsigned int len_b;
